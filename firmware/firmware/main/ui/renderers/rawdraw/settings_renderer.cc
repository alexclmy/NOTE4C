...
/**
 * The left category rail, as a grid rather than as offsets that happen to line
 * up.
 *
 * The arithmetic lives in settings_layout.h, which is header-only and free of
 * every rawdraw and LVGL type so tests/host/test_settings_layout.cc can
 * compile it and assert the invariants. What is aliased here is only the
 * naming, so call sites read the way they always did.
 */
constexpr int kSettingsNavItemH = layout::kNavItemH;
constexpr int kSettingsTableRowH = 34;
/// Shortest a compressed option row may get before the list scrolls instead.
constexpr int kSettingsMinRowH = 28;
...
[[maybe_unused]] void DrawSettingsVectorIcon(uint8_t* fb, int width, const std::string& label,
    ...
    } else if (label == "Clock display") {
        icon_char = "\xef\x80\x97";  // U+F017 clock
    } else if (label == "Storage" || label == "Storage space") {
    ...
    } else if (label == "Chat history length") {
...
void DrawSettingsLayoutDebugOverlay(uint8_t* fb,
    ...
    // Extra top layer only: this does not change layout inputs. The rail
    // geometry is passed in rather than recomputed here, so the overlay cannot
    // draw a grid the page is not actually using.
    const int value_x = content_x + 112;
    const layout::NavPill pill = layout::NavPillFor(nav_divider_x);
    const int nav_pill_x = pill.x;
    const int nav_pill_w = pill.w;
    const int nav_pill_h = layout::kNavPillH;
    const int selected_nav_y = nav_top + current_section_pos * kSettingsNavItemH;
    const int selected_pill_y = layout::NavPillY(nav_top, current_section_pos);
    ...
    DrawDebugDashedHLine(fb, width, selected_nav_y + kSettingsNavItemH / 2,
                         nav_pill_x, nav_divider_x - 6, debug_color);
    ...
    DrawDebugDashedVLine(fb, width, nav_divider_x, body_top + 28, body_bottom, debug_color);
    char divider_buf[16];
    snprintf(divider_buf, sizeof(divider_buf), "x=%d", nav_divider_x);
    DrawText(fb, width, nav_divider_x + 2, body_top + 24, divider_buf, font, debug_color, height);
...
int SettingsRenderer::NavRailWidth() const {
    if (font_ == nullptr) return layout::kRailMinW;
    // "System" is the label the page falls back to when a settings list has no
    // sections at all, so it is part of the measurement either way.
    int widest = MeasureTextWidth("System", font_);
    for (const auto& item : items_) {
        if (item.type != SettingsItemType::Section) continue;
        widest = std::max(widest, MeasureTextWidth(item.label.c_str(), font_));
    }
    return layout::NavRailWidth(widest);
}

void SettingsRenderer::SetAboutInfo(const AboutInfo& info) {
    about_ = info;
    needs_full_refresh_ = true;
}

std::vector<SettingsRenderer::AboutRow> SettingsRenderer::BuildAboutRows() const {
    // "Not read" rather than a blank or a plausible-looking placeholder: a
    // value this page could not obtain has to look different from one it did.
    const std::string unknown = "Not read";
    const auto or_unknown = [&unknown](const std::string& value) {
        return value.empty() ? unknown : value;
    };
    return {
        {"Device", or_unknown(about_.product)},
        {"Firmware", or_unknown(about_.firmware)},
        {"Hardware", or_unknown(about_.hardware)},
        {"LAN address", or_unknown(about_.lan_address)},
        {"Dashboard", or_unknown(about_.pairing)},
        {"Voice", or_unknown(about_.voice)},
        {"Build", or_unknown(about_.build)},
        {"Diagnostic", or_unknown(about_.diagnostic)},
        {"Serial number", mac_address_.empty() ? unknown : mac_address_},
        // Kept, and kept labelled. It is a true fact about where this code came
        // from, and it is not the version of this firmware.
        {"Upstream base", or_unknown(about_.upstream)},
    };
}
...
void SettingsRenderer::Render(uint8_t* fb, int width, int height) {
    ...
    const int nav_divider_x = NavRailWidth();
    const int content_x = nav_divider_x + 16;
    ...
    DrawVLine(fb, width, nav_divider_x,
    ...
    for (int i = 0; i < static_cast<int>(section_indices.size()); ++i) {
        ...
        // One grid: equal margins outside the pill, equal padding inside it,
        // a fixed icon column, and whatever is left for the label.
        const layout::NavPill pill = layout::NavPillFor(nav_divider_x);
        const int nav_pill_x = pill.x;
        const int nav_pill_w = pill.w;
        const int nav_pill_h = layout::kNavPillH;
        const int nav_pill_y = layout::NavPillY(nav_top, i);
        const int icon_x = pill.icon_x;
        const int icon_center_y = sy + kSettingsNavItemH / 2;  // Same center as label
        const int label_x = pill.label_x;
        const int label_max_w = pill.label_max_w;
        // NavRailWidth() already sized the rail so this fits. The fit is kept
        // as a backstop because a label added later must be truncated inside
        // the rail rather than painted across the divider.
        const std::string nav_label = FitTextToWidth(label, font_, label_max_w);
        const int label_y =
            InkCenteredTextTopY(font_, nav_label.c_str(), icon_center_y, kTextOpticalNudgeY);
        if (selected) {
        ...
        DrawText(fb, width, label_x, std::max(body_top + 2, label_y),
                 nav_label.c_str(), font_, nav_fg);
    }
    ...
    if (about_section) {
        const std::vector<AboutRow> rows = BuildAboutRows();
        debug_visible_row_count = static_cast<int>(rows.size());
        // Share the pane between however many rows there are, the same way the
        // options list does. A fixed 34 px row height fitted six rows and
        // silently ran the last four off the bottom of a 300 px panel.
        const int about_available_h = std::max(row_h, body_bottom - kSettingsTableTop - 2);
        const int about_row_h = layout::SharedRowHeight(
            about_available_h, static_cast<int>(rows.size()), row_h,
            static_cast<int>(font_->line_height) + 4);
        int y = kSettingsTableTop;
        for (const auto& row : rows) {
            const int center_y = y + about_row_h / 2;
        ...
        const int option_row_h = layout::SharedRowHeight(available_h, visible_count,
                                                         row_h, kSettingsMinRowH);
    ...
    if (kSettingsLayoutDebugOverlayEnabled &&
        ...
        DrawSettingsLayoutDebugOverlay(fb, width, height, font_,
                                       ...
                                       current_section_pos, nav_divider_x,
...
void SettingsRenderer::RenderItem(uint8_t* fb, int width, int y,
    ...
    if (selected) {
        ...
        //
        // The height tracks the row rather than being a fixed 16 px. The
        // options list compresses its rows to share the pane, so a fixed
        // cursor overhung the row separator on a short row and looked lost on
        // a tall one. The clamping lives in settings_layout.h, where a host
        // test asserts that the bar always stays inside its row.
        const layout::RowCursor cursor =
            layout::RowCursorFor(content_left, row_center_y, row_h);
        DrawRect(fb, width, {cursor.x, cursor.y, cursor.w, cursor.h},
...
void SettingsRenderer::RenderAboutDialog(uint8_t* fb, int width, int height) {
    ...
    const int rows_top_gap = 12;
    // Sized from the rows rather than from a constant, so adding a row cannot
    // push the last one off the bottom edge without anyone noticing. The
    // arithmetic is in settings_layout.h and a host test asserts that the
    // dialog always fits the panel.
    const layout::AboutDialog geometry = layout::AboutDialogFor(
        height, dialog_y, titlebar_h, rows_top_gap, static_cast<int>(rows.size()),
        static_cast<int>(font_->line_height), kAboutRowHeight);
    const int row_h = geometry.row_h;
    const int dialog_h = geometry.height;
    ...
    int y = geometry.rows_top;
    const int rows_x = dialog_x + 70;
    // Only what fits. AboutDialogFor works out how many rows the panel has
    // room for at a legible height; drawing more would paint them off the
    // bottom of the glass, which is what the old fixed-height dialog did.
    const int shown = std::min(geometry.visible_rows, static_cast<int>(rows.size()));
    for (int index = 0; index < shown; ++index) {
        const AboutRow& row = rows[static_cast<size_t>(index)];
...
void SettingsRenderer::RenderStorageDialog(uint8_t* fb, int width, int height) {
    ...
    const char* title = "Storage space";
    const int title_w = MeasureTextWidth(title, font_);
    DrawText(fb, width, dialog_x + (dialog_w - title_w) / 2,
             InkCenteredTextTopYInBox(font_, title, dialog_y, titlebar_h, 0),
             title, font_, text, height);
    ...
    const std::vector<InfoRow> rows = {
        ...
        {"Images", std::to_string(storage_photos_)},
...
void SettingsRenderer::RenderServerDialog(uint8_t* fb, int width, int height) {
    ...
    DrawText(fb, width, dialog_x + 30,
             InkCenteredTextTopY(font_, "UP/DN change  BOOT confirm", hint_center_y, 0),
             "UP/DN change  BOOT confirm", font_, secondary, height);
}
...
void SettingsRenderer::RenderServerListDialog(uint8_t* fb, int width, int height) {
    ...
    const char* title = "Server address history";
    const int title_w = MeasureTextWidth(title, font_);
    DrawText(fb, width, dialog_x + (dialog_w - title_w) / 2,
             InkCenteredTextTopYInBox(font_, title, dialog_y, titlebar_h, 0),
             title, font_, text, height);
    ...
    if (total == 0) {
        DrawText(fb, width, rows_x,
                 InkCenteredTextTopY(font_, "No saved addresses", y + row_h / 2, 0),
                 "No saved addresses", font_, secondary, height);
    } else {
...
void SettingsRenderer::RenderOtaDialog(uint8_t* fb, int width, int height) {
    ...
    if (selecting && !ota_versions_.empty()) {
        ...
        if (status.empty()) status = failed ? "Update failed" : "Fetching version list...";
        status = FitTextToWidth(status, font_, std::max(0, content_right - rows_x));
        DrawText(fb, width, rows_x,
                 InkCenteredTextTopY(font_, status.c_str(), y + row_h / 2, 0),
    ...
    const char* hint = selecting ? "UP/DN select  BOOT update  hold to cancel"
...
void SettingsRenderer::RenderOtaConfirmDialog(uint8_t* fb, int width, int height) {
    ...
    // 确认/取消选项
    const char* options[2] = {"Confirm update", "Cancel"};
    for (int i = 0; i < 2; ++i) {
    ...
    // 提示
    const char* hint = "UP/DN select  BOOT confirm  hold to go back";

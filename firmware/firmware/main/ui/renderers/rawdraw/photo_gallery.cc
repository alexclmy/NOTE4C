...
void PhotoGalleryRenderer::ExitFullscreenMode() {
    if (mode_ != kFullscreenMode) return;
    showing_delete_dialog_ = false;
    mode_ = kMemoryCardMode;
    needs_full_refresh_ = true;
}

void PhotoGalleryRenderer::CloseDeleteDialog() {
    if (!showing_delete_dialog_) return;
    showing_delete_dialog_ = false;
    // Match the dismiss path in HandleInput: the safe answer stays selected so
    // a later reopen does not start on "delete".
    delete_dialog_selected_ = 1;
    needs_full_refresh_ = true;
}
...
void PhotoGalleryRenderer::RenderMemoryCardMode(uint8_t* fb, int width, int height) {
    ...
    if (!photo_ids_.empty()) {
        ...
        const std::string date_text = entry.date[0] ? entry.date : "Date unknown";
        const std::string location_text = entry.location[0] ? entry.location : "Location unknown";
    ...
    if (photo_ids_.empty()) {
        ...
        const char* body = "Push images and captions via /api/push_memory";
        ...
        footer.SetText("UP previous", nullptr, "BOOT details");
    ...
    int y = card_y + 20;
    const char* chip_text = "This day, past years";
    ...
    DrawStyledText(fb, width, chip.x + (chip.w - chip_text_w) / 2,
                   InkCenteredTextTopYInBox(font_, "This day, past years", chip.y, chip.h, 0),
    ...
    const std::string date_label = entry.date[0] ? entry.date : "Date unknown";
    const std::string location_label = entry.location[0] ? entry.location : "Location unknown";
    ...
    footer.SetText("UP/DN page", counter, "BOOT details");
    footer.Draw(fb, width, height);
}
...
void PhotoGalleryRenderer::RenderFullscreenMode(uint8_t* fb, int width, int height) {
    ...
    if (photo_ids_.empty() || !current_photo_data_ || current_photo_size_ == 0) {
        const char* label = "Could not load photo";
        int tw = MeasureTextWidth(label, font_);
...
void PhotoGalleryRenderer::RenderDeleteDialog(uint8_t* fb, int width, int height) {
    ...
    const char* body = "Delete the current photo?";
    ...
    const char* hint = "UP/DN change  BOOT confirm";

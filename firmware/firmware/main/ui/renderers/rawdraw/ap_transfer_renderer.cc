...
void ApTransferRenderer::Render(uint8_t* fb, int width, int height) {
    ...
    // Title
    const char* title = title_text_.empty() ? "Wi-Fi image transfer" : title_text_.c_str();
...
void ApTransferRenderer::RenderInstructions(uint8_t* fb, int width, int height) {
    ...
    const std::string state_hint = !hint_text_.empty()
        ...
        ? "Starting up, you can join the hotspot now"
        : status_message_);
    const std::string ssid_line = "Join " + (ssid_text_.empty() ? std::string("InkScreen-AP") : ssid_text_);
    const std::string pwd_line = password_text_.empty()
        ? std::string("Password: none")
        : ("Password: " + password_text_);
    const char* lines[] = {
        ssid_line.c_str(),
        pwd_line.c_str(),
        "",
    ...
    const char* exit_hint = exit_hint_text_.empty() ? nav::kExitHint : exit_hint_text_.c_str();
...
bool ApTransferRenderer::HandleInput(const ButtonEvent& event) {
    ...
    // AP mode is intentionally stable during slow e-paper refreshes. Leaving it
    // is Back (UP long) or Home (DOWN long), resolved by common/nav_model.cc
    // before this renderer is consulted; short clicks do nothing here.
    if (event.type == ButtonEvent::kBootClick) {
        ESP_LOGI(kTag, "Short click ignored in AP transfer mode");
        return false;
    }
...
void ApTransferRenderer::UseDefaultTransferInstructions() {
    SetInstructionContent("Wi-Fi image transfer",
                          "InkScreen-AP",
                          "12345678",
                          "http://192.168.4.1",
                          "",

...
void WifiRenderer::RenderConnected(uint8_t* fb, int width, int height) {
    ...
    // === Bottom hint ===
    const char* hint = "Press BOOT to go back";
    int hint_w = MeasureTextWidth(hint, font_);
    DrawText(fb, width, (width - hint_w) / 2,
             height - font_->line_height - Style::kSpacingSM, hint, font_, secondary_text);
}
...
void WifiRenderer::RenderDisconnected(uint8_t* fb, int width, int height) {
    ...
    // Primary action
    const char* primary = "Press BOOT to reconnect";
    ...
    // Secondary hint. BOOT long is push-to-talk and does not open Wi-Fi setup;
    // the two-button combo does (common/nav_model.cc, kComboLong).
    const char* secondary_hint = nav::kWifiSetupHint;

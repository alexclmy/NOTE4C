...
std::string Clock::GetDateString(const char* date_format) {
    ...
    // Default: day/month, matching the composed dashboard. Upstream used the
    // Chinese "M月D日" form here.
    char buf2[16];
    snprintf(buf2, sizeof(buf2), "%02u/%02u",
             (unsigned)tm.tm_mday, (unsigned)(tm.tm_mon + 1));

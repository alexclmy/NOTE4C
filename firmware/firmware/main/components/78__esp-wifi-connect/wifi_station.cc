...
// Poulailler dashboard build: no vendor cloud contact.
//
// Upstream probes connectivity by resolving and connecting to whichever remote
// endpoint is configured - the MQTT broker, the WebSocket server, or the OTA
// host (https://ota.zectrix.com/... by default, overridable from NVS "ota_url"
// via the Wi-Fi provisioning page). This device only ever talks to the Mac on
// the LAN, so that probe is an unnecessary third-party connection on every
// association.
//
// Setting an inert CONFIG_OTA_URL would not be enough on its own: the NVS
// override takes precedence over it, and any value previously stored on the
// device would still be contacted. Refusing to resolve any endpoint at all is
// the only version of this that is actually true. The cost is that
// "connectivity confirmed" degrades to "association and DHCP confirmed", which
// is the right trade for a device with no internet-facing job.
static constexpr bool kDashboardDisableCloudProbe = true;
...
static bool ResolveProbeEndpoint(NetworkProbeTarget probe_target,
    ...
    *source = "missing";
    ...
    if (kDashboardDisableCloudProbe) {
        *source = "disabled_dashboard_build";
        return false;
    }

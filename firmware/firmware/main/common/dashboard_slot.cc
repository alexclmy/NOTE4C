/**
 * @file dashboard_slot.cc
 * @brief Implementation of the A/B dashboard frame store.
 *
 * Copyright (c) 2026 NOTE4C poulailler dashboard contributors.
 * SPDX-License-Identifier: MIT
 *
 * See dashboard_slot.h for why this is structured the way it is. No ESP-IDF
 * headers here: the host test suite compiles this same file.
 */
...
constexpr uint8_t kMagic[8] = {'N', '4', 'C', 'D', 'A', 'S', 'H', '1'};

// Header field offsets. Little-endian, fixed width, so a record written by one
// build is readable by another regardless of struct packing rules.
...
bool DashboardSlot::ParseRecord(const uint8_t* raw, size_t len, SlotStatus* out) {
    out->valid = false;

    if (len < kHeaderBytes) {
        return false;
    }
    if (memcmp(raw + kOffMagic, kMagic, sizeof(kMagic)) != 0) {
        return false;
    }
    ...
    out->flags = LoadU32(raw + kOffFlags);
    memcpy(out->sha, raw + kOffSha, kShaBytes);
    return true;
}

bool DashboardSlot::Load() {
    active_ = -1;
    ...
    for (int slot = 0; slot < kSlotCount; ++slot) {
        const int n = io_->ReadSlot(slot, raw, kRecordBytes);
        if (n <= 0) {
            continue;
        }
        SlotStatus st;
...
uint32_t DashboardSlot::active_seq() const {
    return active_ >= 0 ? status_[active_].seq : 0u;
}

uint32_t DashboardSlot::active_source_epoch() const {
    return active_ >= 0 ? status_[active_].source_epoch : 0u;
}

uint32_t DashboardSlot::active_flags() const {
    return active_ >= 0 ? status_[active_].flags : 0u;
}

const uint8_t* DashboardSlot::active_sha() const {
    return active_ >= 0 ? status_[active_].sha : nullptr;
}

bool DashboardSlot::ReadFrame(uint8_t* out) const {
    if (active_ < 0 || io_ == nullptr || out == nullptr || scratch_ == nullptr) {
        return false;
    }

    uint8_t* raw = scratch_;
    const int n = io_->ReadSlot(active_, raw, kRecordBytes);
    if (n <= 0) {
        return false;
    }
    ...
    memcpy(out, raw + kHeaderBytes, kFrameBytes);
    return true;
}

StoreResult DashboardSlot::Store(const uint8_t* payload,
                                 size_t len,
    ...
    if (active_ >= 0 && ConstantTimeEquals(sha, status_[active_].sha, kShaBytes)) {
    ...
    memcpy(raw + kOffMagic, kMagic, sizeof(kMagic));
    StoreU32(raw + kOffVersion, kRecordVersion);
    StoreU32(raw + kOffSeq, seq);
    StoreU32(raw + kOffPayloadLen, static_cast<uint32_t>(kFrameBytes));
    StoreU32(raw + kOffFlags, flags);
    ...
    memcpy(raw + kHeaderBytes, payload, kFrameBytes);

    if (!io_->WriteSlot(target, raw, kRecordBytes)) {
        return StoreResult::kWriteFailed;
    }
    ...
    const int n = io_->ReadSlot(target, raw, kRecordBytes);
    if (n <= 0) {
    ...
    if (verified.seq != seq || !ConstantTimeEquals(verified.sha, sha, kShaBytes)) {
        return StoreResult::kVerifyFailed;
    }

...
void AudioService::AudioInputTask() {
    while (true) {
        ...
        /* Feed the wake word */
        // wake_word_ is never assigned in this firmware: no WakeWord subclass
        // exists on disk and there is no model partition to hold one
        // (PRODUCT-PLAN.md section 2.2). The event bit therefore cannot be set
        // today, but this deref would be the first thing to fault if a wake
        // word were wired carelessly, so it is guarded rather than trusted.
...
const std::string& AudioService::GetLastWakeWord() const {
    // Same reasoning as the feed path above: no wake word is ever installed,
    // so return an empty name instead of dereferencing null.
...
bool AudioService::PlayPcm(const int16_t* samples, size_t count) {
    if (codec_ == nullptr || samples == nullptr || count == 0) {
        return false;
    }
    // An earcon is feedback about a button, so it un-mutes the local sound
    // path the same way PlaySound does. This is the output mute used by
    // MuteSound(), not the push-to-talk software mute, which is a separate
    // setting about the microphone and about speaking responses.
    mute_local_sound_.store(false, std::memory_order_release);
...

    {
        ...
        if (audio_playback_queue_.size() >= MAX_PLAYBACK_TASKS_IN_QUEUE) {
            // Deliberately not waiting on the condition variable. The callers
            // are the push-to-talk hooks on the button task, and a button that
            // stops responding is worse than a tone that does not play.
            ESP_LOGW(TAG, "Playback queue full, dropping %u PCM samples",
...
void AudioService::ClearSendQueue() {
    std::lock_guard<std::mutex> lock(audio_queue_mutex_);
    const size_t dropped = audio_send_queue_.size();
    audio_send_queue_.clear();
    audio_encode_queue_.clear();
    audio_queue_cv_.notify_all();
    if (dropped > 0) {
        ESP_LOGI(TAG, "Dropped %u encoded packets left over from a previous utterance",

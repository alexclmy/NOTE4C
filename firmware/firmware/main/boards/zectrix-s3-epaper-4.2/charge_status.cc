...
void ChargeStatus::Tick(int64_t now_ms) {
    ...
    // The debounce timers are rewritten below and have more than one writer;
    // the state-changed callback is deliberately fired outside this scope, so
    // a callback that reaches back into the board cannot deadlock on it.
    {
    ...
    power_present = (last_power_present_ms_ >= 0) &&
    ...
    no_battery = alt_seen && !detect_stable && !full_stable;
    ...
    // Outside the lock. UpdateSnapshot swaps an atomic, so two concurrent
    // ticks that reach the same state still fire the callback at most once
    // between them.

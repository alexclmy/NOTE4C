...
private:
    ...
    /// Guards the debounce timers below, which are plain members that Tick()
    /// rewrites. The LED task ticks twice a second and the factory test ticks
    /// on demand, so there really are two writers; the snapshot itself is a
    /// packed atomic and needs no lock to read.
